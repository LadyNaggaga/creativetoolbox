// Pizza Shop Ops — hero-tools MCP server on Azure Container Apps.
...
param appName string = 'pizzashop-mcp'

@description('Existing Azure Container Registry name that holds the built image.')
param acrName string

@description('Fully-qualified container image, e.g. myacr.azurecr.io/pizzashop-mcp:latest')
...
// Grant the app's managed identity AcrPull on the registry so it can pull the image.
// (Defined above as `acrPull`, scoped to the user-assigned identity, before the app.)
