#!/usr/bin/env python3
"""
Local Toolbox with Tool Search emulator (pixel-jam-studio).

Serves an MCP-style JSON-RPC endpoint over HTTP so the demo runs with ZERO Azure and zero token.
Faithfully emulates the one behavior the demo is about:
...
The `code_scribe.write_game` hero tool actually writes a real playable index.html to
../game-output/ — that's the "Copilot-SDK-style code emission" moment.
"""
...
if __name__ == "__main__":
    ...
    print(f"listening on http://localhost:{PORT}/toolboxes/pixel-jam-studio/mcp")
