#!/usr/bin/env python3
"""
Squad Room — hero-tools MCP server (cloud).
...
# In the container these live at /app/mock-backends (copied by the Dockerfile) and SQUAD_MOCK_BACKENDS
...
_DEMO_ROOT = _here.parents[2] if len(_here.parents) >= 3 else _here.parent  # demos/squad-room
_DEFAULT_MOCK = _DEMO_ROOT / "mock-backends"
MOCK_DIR = Path(os.environ.get("SQUAD_MOCK_BACKENDS", str(_DEFAULT_MOCK)))
TOOLS_JSON = MOCK_DIR / "tools.json"
HERO_DIR = MOCK_DIR / "hero_tools"

mcp = FastMCP("squadroom", stateless_http=True, host="0.0.0.0", port=int(os.environ.get("PORT", "8000")))
...
if __name__ == "__main__":
    count = build()
    print(f"squadroom MCP server: registered {count} tools from {TOOLS_JSON}")
