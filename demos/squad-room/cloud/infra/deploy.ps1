<#
.SYNOPSIS
  Deploy the Squad Room hero-tools MCP server to Azure Container Apps — no local Docker
...
.EXAMPLE
  ./deploy.ps1 -ResourceGroup rg-squad-cloud -Location eastus
#>
param(
  [Parameter(Mandatory = $true)] [string]$ResourceGroup,
  [string]$Location = 'eastus',
  [string]$AcrName = "squadacr$(([System.BitConverter]::ToString((New-Object Security.Cryptography.SHA256Managed).ComputeHash([Text.Encoding]::UTF8.GetBytes($ResourceGroup))).Replace('-','').ToLower().Substring(0,10)))",
  [string]$ImageTag = 'squadroom-mcp:latest'
)
...
$demoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path   # demos/squad-room
...
Write-Host " then run: azd ai toolbox create squad-room --from-file ./cloud/toolbox.yaml"
Write-Host "===================================================================="
