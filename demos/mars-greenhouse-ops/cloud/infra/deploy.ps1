<#
.SYNOPSIS
  Deploy the Mars Greenhouse Ops hero-tools MCP server to Azure Container Apps — no local Docker
  needed (the image is built server-side with `az acr build`).
...
.EXAMPLE
  ./deploy.ps1 -ResourceGroup rg-mars-cloud -Location eastus
#>
param(
  [Parameter(Mandatory = $true)] [string]$ResourceGroup,
  [string]$Location = 'eastus',
  [string]$AcrName = "marsacr$(([System.BitConverter]::ToString((New-Object Security.Cryptography.SHA256Managed).ComputeHash([Text.Encoding]::UTF8.GetBytes($ResourceGroup))).Replace('-','').ToLower().Substring(0,10)))",
  [string]$ImageTag = 'marsgreenhouse-mcp:latest'
)
...
$demoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path   # demos/mars-greenhouse-ops
...
Write-Host ""
Write-Host " Next: put this URL in cloud/toolbox.yaml (the mars mcp entry server_url),"
Write-Host " then run: azd ai toolbox create mars-greenhouse-ops --from-file ./cloud/toolbox.yaml"
Write-Host "===================================================================="
