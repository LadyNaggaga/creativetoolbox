#!/usr/bin/env python3
"""
Mars Greenhouse Ops — hero-tools MCP server (cloud).
...
with Tool Search. The hero tools reuse the exact logic from mock-backends/hero_tools/; the rest
...
# In the container these live at /app/mock-backends (copied by the Dockerfile) and MARS_MOCK_BACKENDS
...
_DEMO_ROOT = _here.parents[2] if len(_here.parents) >= 3 else _here.parent  # demos/mars-greenhouse-ops
_DEFAULT_MOCK = _DEMO_ROOT / "mock-backends"
MOCK_DIR = Path(os.environ.get("MARS_MOCK_BACKENDS", str(_DEFAULT_MOCK)))
TOOLS_JSON = MOCK_DIR / "tools.json"
HERO_DIR = MOCK_DIR / "hero_tools"

mcp = FastMCP("marsgreenhouse", stateless_http=True, host="0.0.0.0", port=int(os.environ.get("PORT", "8000")))


def _load_hero(tool_name: str):
    """Load the run(args) callable for a hero tool, e.g. 'kb.search_guides' ->
    hero_tools/kb_search_guides.py. Returns None if there's no real implementation."""
...
def _register(tool_name: str, description: str):
    ...
    underscore form (kb_search_guides); the human-facing name stays in the description."""
...
if __name__ == "__main__":
    count = build()
    print(f"marsgreenhouse MCP server: registered {count} tools from {TOOLS_JSON}")
